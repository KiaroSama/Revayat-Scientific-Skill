...
Start from the actual local file, attachment, pasted source or URL the user provided.
A common failure is ingestion — a page times out, a PDF
...
- One shared `terms.tsv` across all sources, so the inferred job and subject lexicon stays consistent. Do not
